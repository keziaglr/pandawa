...
class ArjunaScene: SKScene{
    
    let wayangNode: ArjunaNode = ArjunaNode()
    let wayangNode2: ArjunaNode = ArjunaNode()
    ...
    init(size: CGSize, dataWayang: DataWayang) {
        ...
        if dataWayang.showArjunaDesc == false{
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for node in touchedNodes {
            ...
            if dataWayang.showArjunaDesc == false{
                ...
                }else if node.name == "ArjunaLeftStick" {
                ...
                if  node.name == "ArjunaStickBody" && (120...165).contains(touchLocation.x) {
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if ctr == 3 {
            ...
            dataWayang.showArjunaDesc = true
        }
    }

    
    private func configPhysics(wayangNode: ArjunaNode){
        ...
        let rightArmJoint: SKPhysicsJointPin = SKPhysicsJointPin.joint(
            ...
            bodyB: bodyPhysics,
            anchor: .init(
                x: wayangNode.position.x + wayangNode.body.size.width / 2-6,
        ...
        let leftArmJoint: SKPhysicsJointPin = SKPhysicsJointPin.joint(
            ...
            bodyB: bodyPhysics,
            anchor: .init(
                x: wayangNode.position.x  - wayangNode.body.size.width / 4.3,
                y: wayangNode.position.y + wayangNode.body.size.height / 4.3)
        )
    ...
    private func configShadow(){
        ...
        wayangNode2.leftStick.position = .init(x: -40, y: 16)
        wayangNode2.rightStick.position = .init(x: -41, y: 50)
...
class ArjunaNode: SKSpriteNode{
    ...
    private func createBody(){
        body = SKSpriteNode(imageNamed: "ArjunaBody")
        body.anchorPoint = .init(x: 0.5, y: 0.5)
        body.position = .init(x: 0, y: 6)
        ...
        stickBody.name = "ArjunaStickBody"
    ...
    private func configureBody(){
        ...
        rightArm.position = .init(x: self.body.size.width / 2.3, y:  body.size.height / 4.3)
        ...
        rightHand.position = .init(x: self.body.size.width / 2.3, y: self.rightHand.size.height/20)
        rightHand.name = "ArjunaRightHand"
        ...
        leftArm.position = .init(x: -self.body.size.width / 4.3, y:  body.size.height / 4.3)
        ...
        leftHand.position = .init(x: -self.rightHand.size.width, y: self.rightHand.size.height/20)
        leftHand.name = "ArjunaLeftHand"
        ...
        leftStick.name = "ArjunaLeftStick"
        leftStick.path = path
        ...
        rightStick.name = "ArjunaRightStick"
